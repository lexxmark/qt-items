...
Lines::Lines(const Lines& other)
    : QObject(),
      ...
      m_linesVisibility(other.m_linesVisibility),
      m_linesSizes(other.m_linesSizes),
      m_permutation(other.m_permutation),
      m_isConvertersValid(other.m_isConvertersValid),
      m_convertersCase(other.m_convertersCase),
      m_visToAbs(other.m_visToAbs),
      m_absToVis(other.m_absToVis),
      m_isSizeAtLineValid(other.m_isSizeAtLineValid),
      m_sizeAtLine(other.m_sizeAtLine)
{
}
...
quint32 Lines::findVisLine(quint32 sizeAtLine) const
{
    ...
    if (m_convertersCase == CC_ALL_HIDDEN)
        ...
        return std::min(sizeAtLine / lineSize(0), visibleCount() - 1);
    else
    {
...
quint32 Lines::findVisLineInterval(quint32 sizeAtLine, quint32 startLine, quint32 endLine) const
{
    Q_ASSERT(startLine <= endLine);
    Q_ASSERT(startLine <= (visibleCount()-1));
    Q_ASSERT(endLine <= (visibleCount()-1));
    ...
    else
    {
        if (sizeAtLine >= m_sizeAtLine.back())
            ...
            return findValue(m_sizeAtLine, startLine, endLine, sizeAtLine);
    }
}
